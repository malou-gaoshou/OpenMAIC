...
export function Header({ currentSceneTitle }: HeaderProps) {
  ...
  const handleClickOutside = useCallback(
    ...
    [themeOpen, exportMenuOpen, shareMenuOpen],
  );

  useEffect(() => {
    if (themeOpen || exportMenuOpen || shareMenuOpen) {
      document.addEventListener('mousedown', handleClickOutside);
      return () => document.removeEventListener('mousedown', handleClickOutside);
    }
  }, [themeOpen, exportMenuOpen, shareMenuOpen, handleClickOutside]);
    ...
    <>
      <header className="h-20 px-8 flex items-center justify-between z-10 bg-transparent gap-4">
        ...
        <div className="relative" ref={shareRef}>
          <button
            onClick={() => {
              if (canShare) setShareMenuOpen(!shareMenuOpen);
            }}
            disabled={!canShare}
            title={canShare ? t('share.copyLink') : t('share.notReady')}
            className={cn(
